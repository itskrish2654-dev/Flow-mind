...
export function buildAuthorizationUrl(input: { connectorId: string; redirectUri: string; state: string; codeChallenge: string; scopes: string[]; loginHint?: string | null; selectAccount?: boolean }) {
  ...
  const clientId = registered.manifest.providerFamily === "google" ? process.env.GOOGLE_OAUTH_CLIENT_ID : process.env[`FLOWMIND_CONNECTOR_${input.connectorId.toUpperCase()}_CLIENT_ID`];
  ...
  return registered.manifest.providerFamily === "google" ? addGoogleAuthorizationParameters(url, input) : url;
}
