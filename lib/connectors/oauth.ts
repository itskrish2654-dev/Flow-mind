...
export async function createOAuthAuthorization(input: { userId: string; connectorId: string; scopes?: string[]; returnPath?: string }) {
...
export async function consumeOAuthState(input: { userId: string; connectorId: string; state: string }) {
  ...
  return { verifier, scopes: data.requested_scopes, returnPath: safeOAuthReturnPath(data.return_path), providerFamily: data.provider_family };
}
