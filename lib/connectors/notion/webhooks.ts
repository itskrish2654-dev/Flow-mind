...
export function getInitialNotionVerificationToken(payload: unknown) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) return null;
  const token = (payload as Record<string, unknown>).verification_token;
  if (typeof token !== "string" || token.length === 0 || token.length > MAX_VERIFICATION_TOKEN_LENGTH) return null;
  if (token.trim() !== token || /[\u0000-\u001f\u007f]/.test(token)) return null;
  return token;
}
