...
export function assertPdfScriptSupport(value: string): void {
  if (/\p{Script=Arabic}/u.test(value)) {
    throw new PdfRenderError(
      "Arabic and other right-to-left PDF text are not supported yet because CrazyLoops cannot guarantee correct glyph shaping. Use Latin, Hindi, Chinese, or Japanese text, or remove the Arabic text before generating this PDF.",
    );
  }
}
...
async function fontRuns(value: string, bold: boolean, document: PDFKit.PDFDocument): Promise<FontRun[]> {
  ...
  for (const character of normalized) {
    ...
    if (inRange(codePoint, [{ start: 0x0900, end: 0x097f }])) {
      append(bold ? "CrazyLoopsDevanagariBold" : "CrazyLoopsDevanagari", character);
      continue;
    }
    ...
    append(bold ? "CrazyLoopsLatinBold" : "CrazyLoopsLatin", character);
  }
  return runs;
}
...
export async function generatePdfBuffer(markdown: string): Promise<Uint8Array> {
  ...
  const document = new PDFDocument({
    ...
    bufferPages: true,
    info: { Producer: "CrazyLoops", Creator: "CrazyLoops Native Document Engine" },
  });
  document.registerFont("CrazyLoopsLatin", FONT_PATHS.latin);
  document.registerFont("CrazyLoopsLatinBold", FONT_PATHS.latinBold);
  document.registerFont("CrazyLoopsDevanagari", FONT_PATHS.devanagari);
  document.registerFont("CrazyLoopsDevanagariBold", FONT_PATHS.devanagariBold);
  ...
  for (let index = range.start; index < range.start + range.count; index += 1) {
    ...
    document.font("CrazyLoopsLatin").fontSize(8).fillColor("#64748b").text(
      `Generated by CrazyLoops  |  Page ${index + 1} of ${range.count}`,
