...
export const metadata: Metadata = { title: "Terms of Service | FlowMind", description: "Terms governing use of FlowMind." };

export default function TermsPage() {
  const supportEmail = getSupportEmail();
  return (
    <LegalPageShell eyebrow="Legal" title="Terms of Service" description="A launch draft for using FlowMind. Sections marked for owner review are not final legal facts.">
      <section><h2>Operator and agreement</h2><p className="mt-3">These Terms are between you and <strong>[LEGAL ENTITY NAME — OWNER REVIEW REQUIRED]</strong>, the operator of FlowMind. By creating an account or using FlowMind, you agree to these Terms. The operator’s legal name, address, governing law, and court location require owner/legal review.</p></section>
      ...
      <section><h2>Credentials and security</h2><p className="mt-3">Only provide credentials you are authorized to use. Do not place passwords, API keys, or other secrets in ordinary prompts or public-form fields. FlowMind may suspend access or disable workflows to address security, abuse, legal, or operational risk.</p></section>
      <section><h2>Acceptable use</h2><ul className="mt-3"><li>Do not violate law, privacy rights, intellectual property rights, or third-party terms.</li><li>Do not send spam, malware, deceptive content, or abusive requests.</li><li>Do not probe, bypass, overload, or interfere with authentication, quotas, authorization, CAPTCHA, or service infrastructure.</li><li>Do not use FlowMind for high-risk decisions without qualified human review and appropriate safeguards.</li></ul></section>
      <section><h2>Availability, changes, and termination</h2><p className="mt-3">FlowMind may change, limit, suspend, or discontinue features. The service is provided without a promise of uninterrupted availability. You may delete your account in Settings. FlowMind may suspend or terminate access for material breach, security risk, abuse, or legal necessity.</p></section>
      <section><h2>Intellectual property</h2><p className="mt-3">FlowMind’s software, interface, and service materials are owned by the operator or its licensors. These Terms do not transfer ownership. Rights in user content remain subject to the permissions needed to operate the service.</p></section>
      <section><h2>Disclaimers and liability</h2><p className="mt-3"><strong>[OWNER/LEGAL REVIEW REQUIRED]</strong> To the extent permitted by applicable law, FlowMind is provided “as is” and “as available,” without warranties that cannot legally be excluded. A final limitation-of-liability amount, excluded damages, mandatory consumer protections, and any jurisdiction-specific language must be selected by qualified counsel and are not invented here.</p></section>
      <section><h2>Changes and contact</h2><p className="mt-3">Material Terms changes should be posted with an updated date and, where required, additional notice. Questions may be sent to {supportEmail ? <a href={`mailto:${supportEmail}`}>{supportEmail}</a> : <strong>{SUPPORT_EMAIL_PLACEHOLDER}</strong>}.</p></section>
    </LegalPageShell>
  );
}
