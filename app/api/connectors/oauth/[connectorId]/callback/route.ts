...
export async function GET(request: Request, { params }: { params: Promise<{ connectorId: string }> }) {
  ...
  try {
    const oauth = await consumeOAuthState({ userId: user.id, connectorId, state }); const redirectUri = new URL(`/api/connectors/oauth/${connectorId}/callback`, request.url).toString(); const tokens = await exchangeAuthorizationCode({ connectorId, code, verifier: oauth.verifier, redirectUri, scopes: oauth.scopes }); const admin = createAdminClient();
    const { data: connection, error } = await admin.from("connector_connections").upsert({ user_id: user.id, connector_id: connector.manifest.id, provider_family: connector.manifest.providerFamily, external_account_id: tokens.externalAccountId, external_account_label: tokens.externalAccountLabel ?? null, auth_type: "oauth2", status: "connected", granted_scopes: tokens.scopes, token_expires_at: tokens.expiresAt, last_refreshed_at: new Date().toISOString(), last_error_category: null, updated_at: new Date().toISOString() }, { onConflict: "user_id,connector_id,external_account_id" }).select("id").single();
  ...
  } catch { return NextResponse.redirect(new URL("/settings/connections?error=oauth_callback_failed", request.url)); }
}
