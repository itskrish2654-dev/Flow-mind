...
export const metadata: Metadata = { title: "Security | FlowMind", description: "FlowMind security and trust practices." };

export default function SecurityPage() {
  const supportEmail = getSupportEmail();
  return (
    <LegalPageShell eyebrow="Trust" title="Security at FlowMind" description="A concise, truthful overview of safeguards currently used by the product. No system is perfectly secure.">
      ...
      <section><h2>Application safeguards</h2><ul className="mt-3"><li>Cloudflare Turnstile protects authentication and configured public forms.</li><li>Durable rate limits, usage quotas, and expiring concurrency leases reduce abuse and accidental overload.</li><li>Immutable workflow versions and execution idempotency improve auditability and prevent common duplicate-run failures.</li><li>The execution engine validates capabilities server-side and records explicit success or failure states.</li></ul></section>
      <section><h2>Responsible reporting</h2><p className="mt-3">Please do not access or modify data that is not yours, disrupt service, or publish sensitive findings before remediation. Report a suspected vulnerability to {supportEmail ? <a href={`mailto:${supportEmail}`}>{supportEmail}</a> : <strong>{SUPPORT_EMAIL_PLACEHOLDER}</strong>}. A dedicated security contact and response policy still require owner configuration.</p></section>
      <section><h2>No certification claim</h2><p className="mt-3">FlowMind does not currently claim SOC 2, ISO 27001, HIPAA compliance, formal penetration-test status, or any guarantee of absolute security.</p></section>
    </LegalPageShell>
  );
}
